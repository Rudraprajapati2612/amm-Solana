...
pub mod amm_contract {
    ...
    pub fn addliquidity(
        ...
        if amm.reserve_a == 0 && amm.reserve_b == 0 {
            let product = (amount1 as u128)
                .checked_mul(amount2 as u128)
            ...
            // LP tokens minted proportionally to existing supply
    ...
    pub fn swap(
        ...
        // Verify token_inp is one of the pool's tokens
        require!(
            token_inp == minta || token_inp == mintb,
            AmmError::InvalidToken
        );

        // Verify user_source matches the input token
        require!(
            ctx.accounts.user_source.mint == token_inp,
            AmmError::InvalidToken
        );

        let seeds = &[b"pool", minta.as_ref(), mintb.as_ref(), &[amm.bump]];
        ...
        // Update reserves
        if token_inp == minta {
            amm.reserve_a = amm.reserve_a.checked_add(amount_token_inp).ok_or(AmmError::Overflow)?;
            amm.reserve_b = amm.reserve_b.checked_sub(amount_out).ok_or(AmmError::Overflow)?;
        } else {
    ...
    pub fn removeliquidity(
        ctx: Context<RemoveLiquidityPool>,
        lp_amount: u64,
    ) -> Result<()> {
...
pub enum AmmError {
    ...
    Overflow,
    #[msg("Liquidity ratio is invalid")]
    InvalidRatio,
